    ...
    return (
        <div className="min-h-screen bg-[#F0F4F4] flex flex-col items-center justify-center p-4 font-sans">
            ...
            <div className="text-center mb-8">
                <div className="flex justify-center mb-4">
                    <span className="text-2xl font-black text-sky-800 flex items-center gap-1">
                        RUKKO <span className="text-sky-600 bg-sky-100 px-1 rounded">in</span>
                    </span>
                </div>
                    ...
                    {step === 'input' ? (
                        <>
                            ...
                            <div className="flex bg-gray-100 p-1 rounded-xl mb-8">
                                <button
                                    onClick={() => setLoginMethod('phone')}
                                    className={`flex-1 py-3 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-all ${loginMethod === 'phone' ? 'bg-white text-sky-700 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
                                >
                                    <Phone size={18} /> Phone
                                </button>
                                <button
                                    onClick={() => setLoginMethod('email')}
                                    className={`flex-1 py-3 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-all ${loginMethod === 'email' ? 'bg-white text-sky-700 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
                                >
                                    <Mail size={18} /> Email
                                </button>
                            </div>
                            ...
                            <form onSubmit={handleSendOtp}>
                                <div className="mb-8">
                                    ...
                                    <div className="flex items-center border border-gray-200 rounded-xl px-4 py-3 focus-within:ring-2 focus-within:ring-sky-500 focus-within:border-sky-500 transition-all bg-white">
                                        {loginMethod === 'phone' ? (
                                            <>
                                                <Phone className="text-gray-400 mr-3" size={20} />
                                                <input
                                ...
                                <button
                                    type="submit"
                                    disabled={loading}
                                    className="w-full bg-[var(--color-accent)] hover:bg-[#00796B] text-white font-bold py-4 rounded-xl shadow-lg shadow-sky-500/30 flex items-center justify-center gap-2 transition-all active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed"
                                >
                                    {loading ? (
                                        <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                                    ) : (
                                ...
                                <div className="flex gap-2 justify-center mb-8">
                                    {otp.map((digit, index) => (
                                        <input
                                            ...
                                            onChange={(e) => handleOtpChange(index, e.target.value)}
                                            onKeyDown={(e) => handleKeyDown(index, e)}
                                            className="w-12 h-14 border border-gray-200 rounded-xl text-center text-2xl font-bold text-gray-800 focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20 outline-none transition-all bg-gray-50"
                                            autoFocus={index === 0}
                                        />
                                    ))}
                                </div>
            ...
            <div className="mt-8 text-center">
                <p className="text-gray-500 text-sm">
                    New to Rukkoo.in?{' '}
                    <button onClick={() => navigate('/signup')} className="text-sky-600 font-bold hover:underline">
                        Create Account
                    </button>
                </p>
            </div>
