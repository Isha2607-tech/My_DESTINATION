...
const getAmenityConfig = (item, size = 20) => {
  const name = item?.toLowerCase() || '';
  if (name.includes('wi-fi')) return { icon: <Wifi size={size} />, color: 'text-blue-500', bg: 'bg-blue-50' };
  if (name.includes('pool')) return { icon: <Waves size={size} />, color: 'text-cyan-500', bg: 'bg-cyan-50' };
  if (name.includes('restaurant')) return { icon: <Utensils size={size} />, color: 'text-orange-500', bg: 'bg-orange-50' };
  if (name.includes('service')) return { icon: <BellRing size={size} />, color: 'text-amber-500', bg: 'bg-amber-50' };
  if (name.includes('parking')) return { icon: <ParkingCircle size={size} />, color: 'text-indigo-500', bg: 'bg-indigo-50' };
  if (name.includes('ac')) return { icon: <Wind size={size} />, color: 'text-sky-400', bg: 'bg-sky-50' };
  if (name.includes('tv')) return { icon: <Tv size={size} />, color: 'text-rose-500', bg: 'bg-rose-50' };
  if (name.includes('geyser')) return { icon: <Thermometer size={size} />, color: 'text-red-500', bg: 'bg-red-50' };
  if (name.includes('kitchen')) return { icon: <ChefHat size={size} />, color: 'text-emerald-500', bg: 'bg-emerald-50' };
  if (name.includes('lift')) return { icon: <ArrowUpDown size={size} />, color: 'text-purple-500', bg: 'bg-purple-50' };
  return { icon: <CheckCircle size={size} />, color: 'text-gray-400', bg: 'bg-gray-50' };
};
...
const PropertyDetailsPage = () => {
  ...
  */
  ...
  const handleShare = async () => {
    const shareData = {
      title: property?.name || 'My DESTINATION Stay',
      text: `Check out ${property?.name || 'this amazing place'} on My DESTINATION!`,
        ...
        <div className="bg-white rounded-t-[40px] md:rounded-[40px] shadow-[0_-10px_60px_-15px_rgba(0,0,0,0.15)] p-5 pb-24 md:p-10 min-h-screen md:min-h-fit">
          ...
          <div className="flex flex-col gap-2 mb-4">
            <div className="flex items-center gap-2">
              <span className="bg-[#5F8575]/15 text-[#5F8575] text-[10px] font-black px-3 py-1 rounded-lg uppercase tracking-widest shadow-sm">
                {propertyType}
              </span>
              {rating !== undefined && rating !== null && (
                <div className="flex items-center gap-1 bg-yellow-400/10 text-yellow-700 px-3 py-1 rounded-lg text-[10px] font-black shadow-sm">
            ...
            <div className="flex items-center gap-2">
              <h1 className="text-3xl font-black text-textDark tracking-tight leading-tight">{name}</h1>
              <div className="bg-green-100 p-1 rounded-full shrink-0">
                <BadgeCheck size={18} className="text-green-600 fill-white" />
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5 text-gray-400 text-xs md:text-sm font-medium">
              ...
              <span className="line-clamp-1">
                {address?.city || 'MR10'}, {address?.state || 'City Center'} {address?.fullAddress && `• ${address.fullAddress.split(',')[0]}`}
              </span>
            </div>
          </div>
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar mb-4 pb-1">
            {amenities?.slice(0, 3).map((item, idx) => {
              const { icon, color } = getAmenityConfig(item, 14);
              return (
                <div key={idx} className="flex items-center gap-2 bg-[var(--color-hotel-bg)] border border-gray-100 px-4 py-2.5 rounded-2xl whitespace-nowrap shadow-sm">
                  <div className={color}>{icon}</div>
                  <span className="text-[11px] font-black text-gray-700">{item}</span>
                </div>
              );
            })}
          </div>
          ...
          {/* Description */}
          {/* Description Section with Thumbnail */}
          <div className="mb-4">
            <h2 className="text-lg font-black text-textDark mb-2">About this place</h2>
            <div className="flex gap-4 items-start">
              <p className="flex-1 text-gray-500 leading-relaxed text-xs md:text-sm font-medium line-clamp-4">
                {description || `${name} is one of the best ${propertyType.toLowerCase()} offering luxury, comfort & exceptional hospitality for your perfect stay.`}
              </p>

              {/* Multi-layered Image Stack - Tightened & Interactive */}
              <div className="relative w-24 h-24 shrink-0 -mt-10 mx-1">
                ...
                >
                  <img src={galleryImages[stackIndices[2]] || galleryImages[0]} className="w-full h-full object-cover" />
                </div>
                ...
                >
                  <img src={galleryImages[stackIndices[1]] || galleryImages[0]} className="w-full h-full object-cover" />
                </div>
          ...
          {propertyType === 'Hostel' && config && (
            <div className="mb-4 grid md:grid-cols-2 gap-4">
              <div className="p-4 bg-green-50 rounded-xl">
                <h3 className="font-bold text-green-900 mb-2">Hostel Info</h3>
                <ul className="text-sm text-green-800 space-y-1">
        ...
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-6">
          <div className="flex-shrink-0">
            ...
            <div className="flex items-baseline text-white">
              <span className="text-2xl font-black">₹{priceBreakdown?.grandTotal?.toLocaleString() || bookingBarPrice?.toLocaleString() || 'N/A'}</span>
            </div>
            <p className="text-[9px] text-white/50 font-medium">Extra adult: ₹0 • Extra child: ₹0</p>
          </div>
          <div className="flex-1">
            ...
            >
              {(bookingLoading || checkingAvailability) ? (
                ...
                <>
                  Book Now <ChevronRight size={18} strokeWidth={3} className="group-hover:translate-x-1 transition-transform" />
                </>
              )}
            </button>
          </div>
      ...
      {showOffersModal && (
        <div className="fixed inset-0 z-[60] flex items-end md:items-center justify-center bg-black/60 backdrop-blur-sm p-0 md:p-4 animate-fadeIn">
          <div className="bg-white w-full md:max-w-md md:rounded-2xl rounded-t-2xl max-h-[90vh] flex flex-col shadow-2xl animate-slideUp">
            ...
            {/* Modal Header */}
            <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-[var(--color-hotel-bg)] shrink-0 rounded-t-2xl">
            ...
            <div
              className="p-4 overflow-y-auto overflow-x-hidden space-y-4 bg-[var(--color-hotel-bg)] flex-1 overscroll-y-contain"
              data-lenis-prevent
            >
              {offers.map((offer) => (
                <div
